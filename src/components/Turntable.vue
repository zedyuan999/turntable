			...
			>
				...
				>
					<div
						...
						:style="{
							transform: `rotate(${index * -data.perangle - data.tableRotateangle}deg)`,
						}"
					>
						<p>{{ item._baseIndex }}</p>
						<img
...
export default {
	...
	setup(props: any) {
		...
		const modifyPrev = (index: number) => {
			...
			const listNextIndex = (data.tableList[index]._baseIndex + prevDisance) % props.list.length;
		...
		const modifyNext = (index: number) => {
			...
			const listPrevIndex =
				(data.tableList[index]._baseIndex - (itemNum.value - nextDisance)) % props.list.length < 0
					? props.list.length +
					  ((data.tableList[index]._baseIndex - (itemNum.value - nextDisance)) % props.list.length)
					: (data.tableList[index]._baseIndex - (itemNum.value - nextDisance)) % props.list.length;
		...
		const tableInit = () => {
			...
			for (let i = 0; i < list.length; i++) {
				...
				const index = currIndex < props.list.length ? currIndex : currIndex - props.list.length;
		...
		const getCircleCenterCoordinate = () => {
			...
			const x0 = t.getBoundingClientRect().left + t.getBoundingClientRect().width / 2;
			const y0 = t.getBoundingClientRect().top + t.getBoundingClientRect().height / 2;
			data.circleX = x0;
			data.circleY = y0;
			return { x0, y0 };
		};
		...
		const adjustangle = (adjust: boolean = true) => {
			...
			if (disanceangle > 0) {
				changeangle = gtHalfPerangle ? data.perangle - disanceangle : -disanceangle;
			} else {
				changeangle = gtHalfPerangle ? -data.perangle - disanceangle : -disanceangle;
			}
		...
		const touch = (index: number) => {
			...
			// 是否下半区
			const isUnder =
				(index > data.activeIndex && index <= data.activeIndex + floor(props.list.length / 2)) ||
				(index < data.activeIndex &&
					index <= data.activeIndex + floor(props.list.length / 2) - itemNum.value);
			...
			if (isUnder) {
				...
				data.tableRotateangle +=
					((itemNum.value - index + data.activeIndex) % itemNum.value) * data.perangle;
			}
			...
			if (isUnder) {
				...
				let minNum = aIndex - index < 0 ? aIndex - index + itemNum.value : aIndex - index;
				while (minNum > 0) {
					const res =
		...
		const touchStart = (e: TouchEvent) => {
			...
			data.touched = true;
			const hypotenuse = sqrt(pow(touch.pageX - x0, 2) + pow(touch.pageY - y0, 2)); //斜边
			const angle = acos((touch.pageX - x0) / hypotenuse) / (PI / 180);
			data.baseangle = (touch.pageY - y0 < 0 ? angle : 360 - angle) + data.tableRotateangle;
		};
		...
		const touchMove = (e: TouchEvent) => {
			...
			const touch = e.touches[0];
			const { x0, y0 } = getCircleCenterCoordinate();
			const hypotenuse = sqrt(pow(touch.pageX - x0, 2) + pow(touch.pageY - y0, 2));
			...
			// 控制调整完的角度为正整数
			const endingangle = adjustedangle < 0 ? 360 + adjustedangle : adjustedangle;
