...
export default defineComponent({
	...
	watch: {
		modelValue(val) {
			...
			// 克隆dom
			const { x, y, width: w, height: h } = (this.dom as HTMLDivElement).getBoundingClientRect();
			this.data.ox = x + w / 2;
			this.data.oy = y + h / 2;
	...
	setup(props, { emit }) {
		...
		// 转盘参数
		const tableConfig = reactive<Config>(Object.assign(baseConfig, props.config));
		...
		const adjustangle = (adjust: boolean = true) => {
			...
			if (disanceangle > 0) {
				changeangle = gtHalfPerangle ? tableConfig.itemAngle - disanceangle : -disanceangle;
			} else {
				changeangle = gtHalfPerangle ? -tableConfig.itemAngle - disanceangle : -disanceangle;
			}
			...
			if (data.activeStatus > 0) {
				...
				const baseAngle =
					data.tableRotateAngle - tableConfig.activeAngle + changeangle < 0
						...
						: data.tableRotateAngle - tableConfig.activeAngle + changeangle - 360;
			...
			data.tableRotateAngle =
				abs(data.tableRotateAngle + changeangle) === 360 ? 0 : data.tableRotateAngle + changeangle;
		};
		// 开始滑动
		const touchStart = (e: TouchEvent) => {
			...
			data.startAngle = (pageY - data.oy < 0 ? angle : 360 - angle) + data.tableRotateAngle;
		};
		// 滑动中
		const touchMove = (e: TouchEvent) => {
			...
			data.tableRotateAngle = tableRotateAngle > 0 ? tableRotateAngle - 360 : tableRotateAngle;
			...
			const active: number =
				-(data.tableRotateAngle - tableConfig.activeAngle) / tableConfig.itemAngle;

			if (active < 0 || active > data.dividingActive) {
				data.activeStatus = activeStatus.clockwise;
			} else if (active > props.children!.length - 1 && active <= data.dividingActive) {
		...
		const touchEnd = () => {
			data.touched = false;
			adjustangle();
			const active = -(data.tableRotateAngle - tableConfig.activeAngle) / tableConfig.itemAngle;
